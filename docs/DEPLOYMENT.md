...
- [ ] A LAN client **cannot** use the panel without logging in. Behind
      Caddy/HTTPS the app treats proxied requests as LAN traffic (trusts
      `X-Forwarded-For` only from a loopback peer; uses the rightmost entry).
      The kiosk window stays loopback-exempt on purpose.
- [ ] A forged `X-Forwarded-For: 127.0.0.1` from a LAN device **cannot** bypass
      auth (the proxy appends the real client IP last).
- [ ] Admin account created on first run with a **strong** password (8+ chars;
      change later under **Settings → Change password**).
- [ ] Login rate limit works (5 failures / 60 s per IP → HTTP 429). Counters
      are in-memory and **reset on server restart** — expected.
- [ ] Sessions are in-memory — **every restart logs everyone out**. Apply
      software updates over the LAN, not from the kiosk's own browser.
- [ ] Password is not reused from another system.

### Secrets & data at rest

- [ ] `config.json` is mode `0600` and owned by the control user (not root):
      `sudo ls -l /var/lib/kiosk/config.json`.
- [ ] PCO API key (if used) is treated as an on-device credential. Prefer
...
- [ ] Backup `config.json` and the Chromium profile (services, admin hash, PCO
      session) to a trusted location.

### Network exposure

- [ ] Panel is LAN-only behind HTTPS. Control server: `127.0.0.1:3001`. CDP:
      `127.0.0.1:9222`.
- [ ] Self-signed cert encrypts traffic but does **not** prove identity —
      users accept a warning once per device. Pin/distribute the cert for
      stronger guarantees. Not suitable as a public/cloud deployment by itself.
- [ ] Remote access uses **Tailscale** (ACL-isolated), not port-forwarding.
...
- [ ] Understand that **`kiosk/update.sh` runs as root**. It verifies SHA-256
      and version before install, but a compromised GitHub
...
- [ ] Passwords go to NetworkManager only (never stored/echoed by the kiosk);
      nmcli errors are scrubbed. Note: password is briefly visible in `ps`
      while `nmcli` runs. Connecting can drop the panel's network — operator
      should be on-site or follow the new network.
...
- [ ] **PCO login** — remote control streams the kiosk; complete login from
      the phone
- [ ] **Session persistence** — reboot; kiosk still logged in to Planning
      Center
- [ ] **Service selection** — add a service, tap it, TV shows live/countdown
- [ ] **Display type & theme** — apply both; they stick for the plan
...
- [ ] **Wi-Fi** (Pi + NetworkManager) — section appears; scan + WPA2 connect
      works; Show/Hide behaves; section hidden on Windows/macOS
...
> [!IMPORTANT]
> The kiosk depends on Planning Center's **live URL pattern** and
> **live-controller DOM** (layout/theme selectors). Both can change without
> notice — watch the panel's best-effort "apply" warnings.
